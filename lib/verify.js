...
export default async (pluginConfig, context) => {
  ...
  const { gitlabToken, gitlabAuthHeader, gitlabUrl, gitlabApiUrl, proxy, ...options } = resolveConfig(pluginConfig, context);
  ...
  if (gitlabToken && projectPath) {
    ...
    try {
      if (gitlabAuthHeader === "JOB-TOKEN") {
        logger.warn("GitLab token provided is a Job Token. Some functionality may be limited");
        await got
          .get(urlJoin(projectApiUrl, "releases"), {
            headers: { [gitlabAuthHeader]: gitlabToken },
            ...proxy,
          }).json();
      } else {
        ({
